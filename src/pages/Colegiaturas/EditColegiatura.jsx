...
export default function EditColegiatura() {

    const { colegiaturaId } = useParams();
    console.log("ID:", colegiaturaId)
    const navigate = useNavigate();

    // Stores
    const updateColegiatura = useStoreColegiatura((state) => state.updateColegiatura);
    const colegiatura = useStoreColegiatura((state) => state.colegiatura);
    const fetchColegiaturaById = useStoreColegiatura((state) => state.fetchColegiaturaById);

    const { fetchStudents, students } = useStoreStudents();

    const loading = useStoreColegiatura((state) => state.loading);
    ...
    useEffect(() => {
        if (fetchColegiaturaById) {
            console.log("Fetching colegiatura with ID:", colegiaturaId);
            fetchColegiaturaById(colegiaturaId)
        }
    }, [colegiaturaId, fetchColegiaturaById]);
    ...
    if (!colegiatura) return <p>Colegiatura no encontrado</p>;
    ...
    const onSubmit = async (data) => {
        ...
        toast.promise(
            updateColegiatura(nuevaColegiatura),
            {
                loading: 'Creando Colegiatura...',
                success: () => {
                    showToast('Colegiatura editado con éxito!', 'success');
                    navigate('/colegiaturas');
                    return 'Colegiatura editado con éxito!';
                },
                error: () => {
                    showToast('No se pudo editar la colegiatura.', 'error');
                    return 'No se pudo editar la colegiatura.';
                },
            }
        );
    };
            ...
            <div className="mt-6 h-[600px] overflow-y-auto">
                <Breadcrumbs
                    items={[
                        ...
                        { label: "Editar Colegiatura", link: "/colegiaturas/createcolegiatura" },
                    ]}
                />

                <div>
                    <h2 className="font-bold text-3xl text-gray-500">Editar Colegiatura</h2>
                    <p className="text-gray-600">
                        Complete el formulario para editar una colegiatura.
                    </p>
                </div>
